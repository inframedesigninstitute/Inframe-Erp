...
const initialStudentData: StudentData[] = [
  { name: 'Opon Raries', rollNo: '541.00.00', status: 'Present', secondaryStatus: 'Absent', program: 'B.Tech', Subject: 'CS' },
  { name: 'Nolige Testy', rollNo: '401.00.00', status: 'Absent', secondaryStatus: 'Late', program: 'B.Tech', Subject: 'EE' },
  { name: 'Doyragupens', rollNo: '571.00.00', status: 'Late', secondaryStatus: 'Absent', program: 'M.Tech', Subject: 'CS' },
  { name: 'Dunalule Max', rollNo: '561.00.00', status: 'Present', secondaryStatus: 'Late', program: 'BBA', Subject: 'Civil' },
  { name: 'Alice Smith', rollNo: '123.45.67', status: 'Present', secondaryStatus: 'Present', program: 'B.Tech', Subject: 'ME' },
  { name: 'Bob Johnson', rollNo: '890.12.34', status: 'Absent', secondaryStatus: 'Leave', program: 'MBA', Subject: 'EE' },
  { name: 'Charlie Day', rollNo: '111.22.33', status: 'Late', secondaryStatus: 'Present', program: 'B.Tech', Subject: 'CS' },
];
        ...
        <View style={modalStyles.centeredView}>
          <TouchableWithoutFeedback onPress={() => { /* Prevent closing when tapping inside modal */ }}>
            <View style={modalStyles.modalView}>
              ...
              <Text style={modalStyles.modalBodyText}><Text style={{ fontWeight: 'bold' }}>Subject:</Text> {student.Subject}</Text>
...
const SubjectAttendanceManagement: React.FC = () => {
  ...
  const applyFilters = (data: StudentData[], currentFilters: typeof filters): StudentData[] => {
    return data.filter(student => {
      ...
      const SubjectMatch = currentFilters.Subject === 'Subject' || student.Subject === currentFilters.Subject;
      ...
      return programMatch && SubjectMatch;
    });
  }
    ...
    <View style={styles.mainContainer}>
      <View style={styles.card}>
        ...
        <View style={styles.filterGrid}>
          ...
          {/* Row 2 (2 items) */}
          <ComboboxDropdown
            label="Subject" options={dropdownData.Subject} isText={false}
            selectedValue={filters.Subject} onSelect={(v) => handleFilterSelect('Subject', v)}
            isOpen={openDropdown === 'Subject'} onToggle={() => handleToggleDropdown('Subject')}
            dynamicZIndex={currentZIndex--}
          />
          <ComboboxDropdown
